...
document.addEventListener('DOMContentLoaded', function() {
    const summaryTotalElem5 = document.getElementById('summaryTotal5');

    function updateTotalFromLocalStorage() {
        const grandTotal = localStorage.getItem('grandTotal');
        if (summaryTotalElem5 && grandTotal !== null) {
            summaryTotalElem5.textContent = `S/ ${grandTotal}`;
        } else {
            summaryTotalElem5.textContent = 'Total no disponible'; // Mensaje de fallback
        }
    }

    // Inicializar
    updateTotalFromLocalStorage();
});
